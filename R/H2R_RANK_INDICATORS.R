...
may19 <- dplyr:: select(may19, -contains("_0"))
...
colnames(indics_to_rank)[1] <- "indic_to_rank"
#NEW NAMES OF RANKED DATA
old_new_names <- read_excel("~/Desktop/Nigeria/Hard_to_reach/New_Organization/MayFactsheet/ranked_outputs/INDICATORS_TO_RANK.xlsx", 
                             sheet = "name_change")
old_new_names <- as.data.frame(old_new_names)

#SET WORKING DIRECTORY TO THE RANKING 
ranking_folder <- "~/Desktop/Nigeria/Hard_to_reach/New_Organization/MayFactsheet/ranked_outputs"
setwd(ranking_folder)

#LIST OF INDICATORS TO RANK
indics_to_rank <- as.character(indics_to_rank$indic_to_rank)
...
update_ranked_names <- function(ranked_data){ 
...
#################################RANK FUNCTION######################################
#OUTPUTS CSVs OF RANKED INDICATORS
###NOTE: NEED "old_new_names" file with old name-option strings and their "clean" definitions
...
#"indics_to_rank" == A character vector of the names of indicators to rank
rank_indicators <- function(dataset, geo_level, percent_count, processed_outputs_folder,indics_to_rank_df) {
 ...
 #COUNT RESPONSES PER_QUESTION
  ...
  #RANK
  ranked <- rank_delux(dataset,indics_to_rank[i],percent_count,top_x,indics_to_rank[i],geo_level)
  ranked <- update_ranked_names(ranked)
  write.csv(ranked,paste0(processed_outputs_folder,"/",indics_to_rank[i],".csv"))
  }
}
#RUN--SHOOTS CSVs TO FOLDER
rank_indicators(may19,"LGA","percent", "processed_ranked_outputs",indics_to_rank)
